<?php
	...
	{
		$path="news_editor/upload".strrchr($new_content[1][$i],"/");
		if(unlink($path))
		{
	...
	if(mysql_query("delete from tb_highlights where id='".$_GET[id]."'",$conn)){
	  echo "<script>alert('Delete Highlights successfully!');history.back();</script>";
	}else{
	   echo "<script>alert('Failed to delete highlights!');history.back();</script>";
	}
?>