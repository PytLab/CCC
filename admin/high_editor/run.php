...
if(mysql_query("insert into tb_news(content,title,author,createtime) values ('".$news_content."','".$news_title."','".$author_name."','".$createtime."')",$conn))
 {
 echo "<script>alert('新闻发表成功!');window.history.go(-2);</script>";
 mysql_close($conn);
 }
 else
 {
   echo "<script>alert('新闻发表失败!');window.history.go(-2);</script>";
   mysql_close($conn);
 }