...
if($_SESSION["admin_nc"]=="")
 {
  ...
  exit;
 	...
 	$id=$_GET[id];
 	$sql=mysql_query("select * from tb_highlights where id='".$id."'",$conn);
    $info=mysql_fetch_array($sql);//获取该博客的信息
	$title=$info[title];
	$content=$info[content];
	$pubtime=$info[pub_time];
?>
...
var contT = '<?php echo addslashes($content); ?>';         //待编辑文本
var formT='<div id="news_title"><p style="color:#999;">Highlight标题</p><input name="subject" id="subject" value="<?php echo addslashes($title); ?>" type="text"  maxlength="180"/><input type="hidden" name="blog_id" value="<?php echo $id;?>" /><br /><p style="color:#999;font-family:Microsoft YaHei;">发表时间</p><input name="pubtime" id="subject" value="<?php echo $pubtime;?>"  type="text"  maxlength="180"/ ><p style="color:#999;">内容(必填)</p></div>';
var path = '';          //相对路径
