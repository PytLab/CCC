...
//过滤主题
function filter1($text, $no_filter_key = false) {
  $text = trim($text);
  ...
  $text = str_replace('\'', '&#039;', $text);
  $text = str_replace('\\', '&#92;', $text);
  if ($no_filter_key == false) {
    filter_badwords($text);
  }
  //$text = str_replace(' TMD ', ' ＴＭＤ ', $text); // |TMD| 做了title FULLTEXT 分隔符了
  return $text;
}
...
function server_sbj($str) {
  if ($str == '')
    die('题目出现空项！问题分析：1、你没添写 2、程序传递出错。');
  if (($n = strlen($str)) && $n > 180)
    die('提交被拒绝！当前题目超过限度180个字符！（即限汉字60个，英文180个）<br />你要减少约'.ceil(($n - 180) / 3).'汉字，或减少约'.($n - 180).'个英文。');
}
...
function server_chk($str,$len) {
  if (filter_result_check($str) == '')
    die('提交被拒绝！问题分析：1、你没添写 2、您并未输入有效的文本内容，如：你只输入了空格、空html标签等。');
  if (abs($len) > 0) {
    if (strlen($str) > $len)
      die('提交被拒绝！字数超出限定数量'.$len.'。');
  }
}

//过滤词汇
function filter_badwords($text) {
  if ($badwords = @file('js/badword.txt')) {
    foreach ($badwords as $badword) {
	  ...
	  if ($badword != '') {
        ...
        if (preg_match('/'.$badword.'/i', $text, $matches))
          die('提交被拒绝！有禁用词汇“'.preg_replace('/[0-9a-z]|[\x80-\xff]{3}/iU', '$0<span style="color:#FFFFFF;display:none;">禁</span>', $matches[0]).'”。请<a href="javascript:void(0)" onclick="window.history.back();return false;">返回</a>检查。');
        unset($matches);
      }
    }
  }
